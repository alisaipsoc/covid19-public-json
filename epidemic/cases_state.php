...
/**
 * Retrieve all dates available in cases_state.csv
 * 
 */
...
foreach ($dates as $date) {
    for ($i = 0; $i < count($data); $i++) {
        if ($data[$i][0] == $date) {
            $cases_state[$date][$data[$i][1]] = $data[$i][2];
        }
    }
}

/**
 * Encode the cases_state as a JSON.
 * 
 */
$cases_state = json_encode($cases_state, JSON_PRETTY_PRINT);
